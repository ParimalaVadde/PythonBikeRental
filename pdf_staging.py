...
def transform_dataframes(transformed_df, ss, glueContext, compute_uuid_udf, utl):
    ...
    transformed_contacts = transformed_json_df.select(*ss.contacts).distinct().withColumnRenamed(
    ...
    transformed_spend_analysis = transformed_spend_analysis.withColumnRenamed(
		"stg_business_entity_id", "stg_payee_business_entity_id").withColumn(
		"analysis_conducted_dt",when(col("analysis_conducted_dt").isNotNull(),to_date(col("analysis_conducted_dt").substr(1, 8), "yyyyMMdd"))).withColumn(
		"period_start_date",when(col("period_start_date").isNotNull(),
